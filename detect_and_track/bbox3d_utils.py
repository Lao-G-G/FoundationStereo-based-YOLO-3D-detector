...
class BBox3DEstimator:
    ...
    """
    def __init__(self, camera_matrix=None, projection_matrix=None, class_dims=None, use_metric_depth=False):
        """
        ...
        Args:
            ...
            use_metric_depth (bool): If True, depth_value is in meters (from stereo).
                                      If False (legacy), depth_value is normalized 0-1.
        """
    ...
    def estimate_3d_box(self, bbox_2d, depth_value, class_name, object_id=None):
        ...
        if self.use_metric_depth:
            # depth_value is already in meters (from stereo depth)
            distance = max(depth_value, 0.5)  # Minimum 0.5m to avoid degenerate cases
        else:
            # Legacy: map depth_value (0-1) to a range of 1-10 meters
    ...
    def _backproject_point(self, x, y, depth):
        ...
        if not self.use_metric_depth:
            # Legacy: adjust the y-coordinate for indoor scenes
            # With metric depth, we use the true backprojection
    ...
    def draw_box_3d(self, image, box_3d, color=(0, 255, 0), thickness=2):
        ...
        depth_value = box_3d.get('depth_value_metric', box_3d.get('depth_value', 0.5))
        ...
        # For metric depth (meters), normalize to 0-1 range for visual offset
        if self.use_metric_depth and depth_value > 1.0:
            # Metric depth: map to 0-1, assume max 80m range
            viz_depth = min(depth_value / 80.0, 1.0)
        else:
            viz_depth = depth_value
        
        offset_factor = max(0.0, 1.0 - viz_depth)
        ...
        if 'depth_value_metric' in box_3d:
            depth_m = box_3d['depth_value_metric']
            depth_method = box_3d.get('depth_method', 'unknown')
            depth_text = f"D:{depth_m:.2f}m ({depth_method})"
            cv2.putText(image, depth_text, (x1, text_y),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            text_y -= 15
        elif 'depth_value' in box_3d:
...
class BirdEyeView:
    ...
    """
    def __init__(self, size=(400, 400), scale=30, camera_height=1.2, use_metric_depth=False):
        """
        ...
        Args:
            ...
            use_metric_depth (bool): If True, depth is in meters; if False, normalized 0-1.
        """
    ...
    def reset(self):
        ...
        for dist in [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]:
            ...
            thickness = 2 if (dist % 1 == 0) else 1
            cv2.line(self.bev_image, 
                    (self.origin_x - 5, y), 
                    (self.origin_x + 5, y), 
    ...
    def draw_box(self, box_3d, color=None):
        ...
        try:
            ...
            depth_value = box_3d.get('depth_value_metric', box_3d.get('depth_value', 0.5))
            if self.use_metric_depth:
                # depth_value is already in meters
                depth = max(depth_value, 0.5)
            else:
                # Legacy: map depth value (0-1) to a range of 1-5 meters
